...
void man(char *name)
{
    ...
    {
        printf("Description : Used to read data frm regular file\n");
...
int truncate_File(char *name)
{
    ...
    memset(UFDTArr[fd].ptrfiletable->ptrinode->Buffer,0,1024);  // sets the first count bytes of dest to the value c
    UFDTArr[fd].ptrfiletable->readoffset=0;
    UFDTArr[fd].ptrfiletable->writeoffset=0;
    UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize=0;   return 0;
}
...
int main()
{
    ...
    while(1)
    {
        ...
        count=sscanf(str, "%s %s %s %s", command[0], command[1], command[2], command[3]);   //returns the number of fields that were successfully converted and assigned.
        ...
        else if(count==4)
        {
            if(strcmp(command[0], "lseek")==0)
            {
                ...
                ret=LseekFile(fd,atoi(command[2]),atoi(command[3]));        // atoi - converting a string value to an integer value.
                if(ret==-1)
                {
                    printf("Error : Unaable to perform lseek\n");
                }
            }
            else
            {
